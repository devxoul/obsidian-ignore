...
describe('computeIgnored', () => {
  ...
  it('matches globstar patterns at root and nested paths', () => {
    expect(computeIgnored('**/foo.md', ['a/b/foo.md', 'foo.md', 'foo.txt'])).toEqual(['a/b/foo.md', 'foo.md'])
  })

  it('re-includes negated paths', () => {
    expect(computeIgnored('build/*\n!build/keep.md', ['build/x.md', 'build/keep.md'])).toEqual(['build/x.md'])
  })
...
describe('compactToFolders', () => {
  ...
  it('keeps individual files for a partially ignored folder', () => {
    expect(compactToFolders(['src/generated.md'], ['src/generated.md', 'src/main.ts'])).toEqual(['src/generated.md'])
  })
